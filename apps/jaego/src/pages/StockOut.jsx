...
// ── 공통 폼 UI ───────────────────────────────────────────────────────────────
function StockOutForm({ user, onSuccess }) {
  ...
  const currentStock = selected?.stock?.[0]?.quantity ?? 0
  ...
  async function handleSubmit(e) {
    ...
    if (qty > currentStock) { setError(`현재 재고(${currentStock}${selected.unit})를 초과할 수 없습니다.`); return }
    setSubmitting(true)
    setError('')
    setSuccess('')
    try {
            ...
            {favorites.map(p => {
              ...
              return (
                <button
                  ...
                  style={{
                    background: isSelected ? 'var(--color-out-light)' : 'var(--color-white)',
                    border: `2px solid ${isSelected ? 'var(--color-out)' : 'var(--color-border)'}`,
                ...
                >
                  ...
                  <div style={{ fontSize: 12, color: stock === 0 ? 'var(--color-out)' : 'var(--color-text-sub)' }}>
                    재고 {stock}{p.unit}
                  </div>
                </button>
              )
            ...
            {results.map((p, idx) => (
              <button
                ...
                style={{
                  ...
                  padding: '10px 14px',
                  background: idx === activeIndex ? 'var(--color-out-light)' : 'none',
                  border: 'none',
      ...
      {selected && (
        <div style={{
          ...
          <div>
            ...
            <div style={{ fontSize: 13, color: currentStock === 0 ? 'var(--color-out)' : 'var(--color-text-sub)', marginTop: 2 }}>
              현재 재고: {currentStock}{selected.unit}
              {currentStock === 0 && ' (재고 없음)'}
            </div>
          </div>
          <button
            onClick={resetForm}
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ marginBottom: 16 }}>
          <label style={{ fontSize: 13, color: 'var(--color-text-sub)', display: 'block', marginBottom: 6 }}>
            수량
            {selected && <span style={{ color: 'var(--color-text-sub)', marginLeft: 8 }}>최대 {currentStock}{selected.unit}</span>}
          </label>
          <input
            type="number"
            min={1}
            ...
            onChange={e => {
              ...
              setQty(v)
              if (selected && v > currentStock) {
                setError(`현재 재고(${currentStock}${selected.unit})를 초과할 수 없습니다.`)
              } else {
                setError('')
              }
            }}
            style={{
              width: '100%',
              padding: '12px 14px',
              border: `1px solid ${selected && qty > currentStock ? 'var(--color-out)' : 'var(--color-border)'}`,
              borderRadius: 'var(--radius)',
              fontSize: 20,
              fontWeight: 700,
              background: 'var(--color-white)',
        ...
        <button
          ...
          style={{
            width: '100%',
            padding: '14px',
            background: submitting ? '#9CA3AF' : 'var(--color-out)',
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
          <thead>
            <tr style={{ background: 'var(--color-bg)' }}>
              {['일시', '상품명', '수량', '판가', '합계', '메모'].map(col => (
                <th key={col} style={{
                  padding: '10px 16px',
                  textAlign: ['수량', '판가', '합계'].includes(col) ? 'right' : 'left',
                  fontWeight: 600,
                  fontSize: 12,
          ...
          <tbody>
            {logs.map(l => (
              <tr key={l.id} style={{ borderBottom: '1px solid var(--color-border)' }}>
                ...
                <td style={{ padding: '12px 16px', color: 'var(--color-text-sub)', whiteSpace: 'nowrap', textAlign: 'right' }}>
                ...
                <td style={{ padding: '12px 16px', fontWeight: 600, color: 'var(--color-text)', textAlign: 'right' }}>
                  {l.selling_price > 0 ? `₩${(l.selling_price * l.quantity).toLocaleString()}` : '-'}
                </td>
          ...
          <tfoot>
            <tr style={{ background: '#FFF1F2', borderTop: '2px solid var(--color-out)' }}>
              ...
              <td style={{ padding: '10px 16px', fontWeight: 700, color: 'var(--color-out)', textAlign: 'right' }}>
...
export default function StockOut() {
  ...
  const { logs, loading: logLoading, fetchLogs } = useStockLog(null, 'out')
      ...
      <div style={{ background: 'var(--color-bg)', minHeight: '100vh' }}>
        <header style={{
          ...
          <button
            ...
            aria-label="뒤로가기"
          >←</button>
          <span style={{ fontWeight: 700, fontSize: 18, color: 'var(--color-out)' }}>출고</span>
        </header>
        <div style={{ maxWidth: 480, margin: '0 auto', padding: '16px 16px 40px' }}>
          <StockOutForm user={user} />
        </div>
      </div>
    )
  }
    ...
    <div style={{ background: 'var(--color-bg)', minHeight: '100vh' }}>
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <button
            ...
            aria-label="뒤로가기"
          >←</button>
          <span style={{ fontWeight: 700, fontSize: 20, color: 'var(--color-out)' }}>출고</span>
        </div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <StockOutForm user={user} onSuccess={fetchLogs} />
        </div>
